...
class RebuildConfig:
    ...
    # pointer 供 claude -p runner 使用，仅更新会话指针。
    activation: str = "tmux"
    systemd_unit: str = "claude-session.service"

    resume_pointer_path: str = "/etc/kael-thread-rebuild/resume_session"
    ...
    def from_mapping(cls, raw: dict[str, Any]) -> "RebuildConfig":
        ...
        config = cls(
            ...
            systemd_unit=str(raw.get("systemd_unit", "claude-session.service")).strip(),
            resume_pointer_path=str(raw.get("resume_pointer_path", "/etc/kael-thread-rebuild/resume_session")).strip(),
