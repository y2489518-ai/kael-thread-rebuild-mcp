...
def estimate_tokens(text: str) -> int:
    """粗估一段文本的 token 数。CJK ×1.1，其余 ÷1.5。

    原来全文一律 `len(text) // 3`，那是英文散文的经验值（约 4 字符/token）。真实文本
    两头都不符合：中文一个字就要一个多 token，而非中文那部分也不是英文散文，是标点、
    路径、代码、符号——token 密度远高于 4 字符/token。两头都低估，合起来差一倍。

    **系数是拿 Claude Code `/context` 的官方读数反解的**（0815），两个样本：

    | 文件 | 字符 | 其中 CJK | 官方 token |
    |---|---:|---:|---:|
    | CLAUDE.md | 4,218 | 2,035 | 3.6k |
    | MEMORY.md | 4,338 | 1,154 | 3.2k |

    解出 CJK ≈ 1.13、其余 ≈ 0.6。取 1.1 与 1/1.5，两个样本分别 +2.6% 和 +6%——
    **都落在高估一侧，这是故意的**：这个数唯一的用途是 `carry_max_tokens` 防炸上限，
    低估会真把窗撑爆，高估只是早一点提醒该续窗。

    ⚠️ 别拿早先那次「自测差 1%」当验证：那是拿本函数的旧版去对另一个同样由我估出来的
    数字，两边同源，等于自己验自己。真正的外部基准只有 `/context` 的读数。
    """
    cjk = len(_CJK_RE.findall(text))
    return int(cjk * 1.1 + (len(text) - cjk) / 1.5)
