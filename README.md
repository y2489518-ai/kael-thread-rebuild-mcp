...
> **别拿 `estimated_tokens` 当水位表看**（0815 实测修正）。这个数只统计"搬过去的对话"，而新窗口一睁眼的真实上下文里还压着一大块**固定开销**——工具定义、每个 MCP server 的须知、CLAUDE.md、记忆索引。实测同一台机器上，这块固定开销昨晚是 5.9 万、今天是 8.8 万 token，**它不是常量**，挂几个 MCP 就抬几分。
>
> 所以「搬进来 9 万」对应的开口水位是 15.7 万，不是 9 万。要判断"还能撑多久"，得把这块加上，而这块只能从会话自己的 usage 里读，不在本工具的视野内。
...
| 三 零机器裁决，不打分不摘要 | 有。差异：超硬上限时他家 fail 给人工，本项目从最老整轮开始丢并如实计数（默认 `carry_max_tokens = 0` 不限，不会触发） |
