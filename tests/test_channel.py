...
def test_group_chat_and_reactions_are_not_speech():
    """Test group chat and reactions are not speech."""
    assert assistant_text(channel_reply("大家好", tool="mcp__example__group_send_message")) == ""
...
def test_terminal_text_and_channel_reply_both_kept():
    ...
    event["message"]["content"].append(
        {"type": "tool_use", "name": "mcp__companion__reply", "input": {"chat_id": "me", "text": "通道内的文本"}}
    )
    text = assistant_text(event)
    assert "终端里说的" in text and "通道内的文本" in text


def test_complete_channel_conversation_survives():
    """Preserve all turns in a channel conversation."""
    rows = [
        channel_user("任务状态?"),
        channel_reply("我在"),
        channel_user("连接异常了"),
        ...
        tool_result("一堆进程"),
        channel_reply("已恢复 连接可用了"),
        channel_user("本次需要处理多个任务"),
        channel_reply("请提供任务列表"),
    ]
    turns = conversation_turns(rows, 0)
    assert len(turns) == 3, "三个回合全在，运行痕迹不打断回合"
    assert "任务状态?" in turns[0].text and "我在" in turns[0].text
    assert "连接异常了" in turns[1].text and "已恢复" in turns[1].text
    ...
    assert "本次需要处理多个任务" in carried
    assert "请提供任务列表" in carried


def test_queued_channel_message_is_restored():
    """Test queued channel message is restored."""
    row = queued_channel("请继续执行当前任务")
    restored = restore_queued_input(row)
    assert restored is not None
    assert restored["type"] == "user"
    assert restored["isMeta"] is False
    assert "请继续执行当前任务" in restored["message"]["content"]
...
def test_queued_slash_command_is_not_speech():
    """Test queued slash command is not speech."""
...
def test_a_message_queued_mid_turn_lands_exactly_once(tmp_path):
    ...
    assert "请修改这段代码" in blob


def test_channel_opening_means_no_startup_snapshot():
    """Test channel opening means no startup snapshot."""
    rows = [
        channel_user("任务状态?"),
        channel_reply("我在"),
