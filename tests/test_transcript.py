...
def test_consecutive_human_messages_all_survive():
    """Test consecutive human messages all survive."""
    rows = [
        user("第一条"),
        user("第二条"),
        user("第三条"),
...
def test_unanswered_tail_is_kept_by_default():
    """Test unanswered tail is kept by default."""
...
def test_carry_overflow_block_keeps_every_turn_and_flags():
    """Test carry overflow block keeps every turn and flags."""
    rows = []
    for index in range(10):
        rows.append(user(f"问题 {index} " + "字" * 4000))
        rows.append(assistant(f"回答 {index} " + "字" * 4000))
...
def test_matches_the_official_context_readout():
    """Test matches the official context readout."""
...
def test_same_length_chinese_still_costs_more_than_ascii():
    zh = "用户说的每一句都要原样搬过去" * 20
    en = "every test message should be retained" * 20
    n = min(len(zh), len(en))
...
def test_mixed_text_is_between_the_two_extremes():
    mixed = "用户说 rebuild 之后要 grow 一下" * 30
...
def test_turn_token_estimate_uses_the_weighted_rule():
    ...
    rows = [user("请复述上一轮任务要求"), assistant("上一轮要求已记录")]
